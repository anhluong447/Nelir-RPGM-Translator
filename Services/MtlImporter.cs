...
namespace Nelir.Services
{
    public class MtlImporter
    {
        ...
        public int ImportSingleMtlFile(string file, ProjectState projectState)
        {
            int mergeCount = 0;
            try
            {
            ...
            {
                Console.WriteLine($"Error importing single MTL file {file}: {ex.Message}");
            }
            return mergeCount;
        }
        ...
        public int ImportMtlFolder(string mtlFolderPath, ProjectState projectState)
        {
            ...
            try
            {
                ...
                {
                    mergeCount += ImportSingleMtlFile(file, projectState);
                }
            }
            catch (Exception ex)
            {
